...
function addHint(field) {
	...
	if( field.value == "" || field.value == titleHint ) {
			//in "password" inputs, set to "text" to show hint, preserve type in class propibute
...
//based on http://arjansnaterse.nl/changing-type-propibute-in-ie
...
function isFilled(input) {
	...
	if( input.prop('title').indexOf("**")==0) {
		var hintText = input.prop('title').substring(2);
	} else if( input.prop('title').indexOf("*")==0) {
		var hintText = input.prop('title').substring(1);
	}//end clear hints
...
function isTypeValid( type, value ) {
	...
	if( type == classprefix + 'Mail' ) {
		...
		//var emailFilter=/^.+@.+\..{2,}$/;
		var emailFilter=/^[_A-Za-z0-9-]+(\.[_A-Za-z0-9-]+)*@[A-Za-z0-9-]+(\.[A-Za-z0-9-]+)*(\.[A-Za-z]{2,3})$/;
		var illegalChars= /[\(\)\<\>\,\;\:\\\/\"\[\]]/		
